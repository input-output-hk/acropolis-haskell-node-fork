...
startStep epochNo slotsPerEpoch b@(BlocksMade b') es'@(EpochState acnt ls' ss nm) maxSupply asc secparam =
  let SnapShot stake delegs poolParams = ssStakeGo ss
      ...
      -- ls,es are not necessary anymore
...
createRUpd epochNo slotsPerEpoch blocksmade epstate maxSupply asc secparam = do
  let step1 = startStep epochNo slotsPerEpoch blocksmade epstate maxSupply asc secparam
