...
rupdTransition = do
  TRC (RupdEnv b es _tracking, ru, s) <- judgmentContext
    ...
    RewardsJustRight ->
      case ru of
        SNothing ->
          ...
          (pure . SJust) (startStep e slotsPerEpoch b es maxsupply asc k)
    ...
    RewardsTooLate ->
      case ru of
        SNothing -> do
          ...
          let pulser = startStep e slotsPerEpoch b es maxsupply asc k
